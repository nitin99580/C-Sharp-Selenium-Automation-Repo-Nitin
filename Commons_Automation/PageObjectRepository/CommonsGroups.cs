...
namespace Commons_Automation
{
    public class CommonsGroups : MainSetup
    {
        ...
        [FindsBy(How = How.XPath, Using = "//*[@class='entity-intro__action']/a")]
        public IWebElement member { get; set; }

        [FindsBy(How = How.XPath, Using = "//*[@id='membership']/li/a[contains(text(),'Leave')]")]
        public IWebElement leaveGroupLink { get; set; }


        [FindsBy(How = How.XPath, Using = "(//div[@Class='card-action']//a)[2]")]
        public IWebElement MembershipStatus { get; set; }

        [FindsBy(How = How.XPath, Using = "//div[@Class='card-action']/span/a[text()='Join']")]
        public IWebElement JoinLink { get; set; }

        [FindsBy(How = How.XPath, Using = "//div[@Class='card-action']/span/a[text()='View']")]
        public IWebElement ViewLink { get; set; }

        [FindsBy(How = How.CssSelector, Using = "#group-list > div > div > div > div> ul > li")]

        [FindsBy(How = How.XPath, Using = "//*[@class='field-content']//span")]
        public IList<IWebElement> groupList { get; set; }

        [FindsBy(How = How.XPath, Using = "(//*[@class='card-action']//a)[2]")]
        public IWebElement JoinMembershipButton { get; set; }
        ...
        [FindsBy(How = How.CssSelector, Using = "#views-exposed-form-group-members-page-1 > ul > li > div.collapsible-header > a")]
        public IWebElement filterMembers { get; set; }
        ...
        [FindsBy(How = How.XPath, Using = "(//button[contains(text(),'configuration')])[1]")]

        public IWebElement EditLastPostBtn { get; set; }

        [FindsBy(How = How.XPath, Using = "(//*[contains(@class,'contextual-links')])[1]//a[text()='Edit']")]
        ...
        [FindsBy(How = How.XPath, Using = "(//*[@title='Add a comment'])[2]")]
        ...
        public void JoinConversationGroup(string GroupName)
        {
            ...
            foreach (IWebElement group in groupList)
            {
                ...
                if (cardLines.Count() > 0)
                {
                    ...
                    if (cardLines[0].ToLower().Equals(GroupName.ToLower()) && MembershipStatus.Text.Contains("Join"))
                    {
                        Console.WriteLine("Group Name : " + cardLines[0]);
                    ...
                    if (cardLines[0].ToLower().Equals(GroupName.ToLower()) && MembershipStatus.Text.Contains("Private"))

                    //if (cardLines[0].ToLower().Equals(GroupName.ToLower()) && JoinMembershipButton.Text.Contains("Private"))
                    ...
                    if (cardLines[0].ToLower().Equals(GroupName.ToLower()) && MembershipStatus.Text.Contains("You are a member"))

                    {
                        Console.WriteLine("Group Name : " + cardLines[0]);
                        IWebElement viewGoup = group.FindElement(By.XPath("(//*[@class='card-action']//a)[1]"));
        ...
        {
            //payButton.Click();
            PageServices.ClickButtonByJavaScript(driver, payButton);
        ...
        public void VerifyASCPaymentFieldsPaymentPage()
        {
            ...
            //PageServices.ScrollIntoElement(driver, SelectASCCostDurationValue);
            Actions actions1 = new Actions(driver);
            actions1.MoveToElement(SelectASCCostDurationValue);
            //SelectASCCostDurationValue.Click();
            PageServices.ClickButtonByJavaScript(driver, SelectASCCostDurationValue);
            SelectASCProofDuration(ascDuration);
            driver.SwitchTo().Frame("asc_proofreader_iframe");
            driver.WaitUntilElementClickable(inputCreditCardNumber, 120);
            Actions actions = new Actions(driver);
